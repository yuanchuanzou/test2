...
\documentclass[prd,twocolumn,amsmath,amssymb,aps]{revtex4-2} %nofootinbib
%\documentclass[aps,prl,twocolumn,superscriptaddress,showpacs,preprintnumbers,amsmath,amssymb,linenumbers]{revtex4-2}
% \documentclass[10pt,prd,preprintnumbers,floatfix,aps,nofootinbib,showpacs,twocolumn,superscriptaddress,noshowpacs]{revtex4-2} %twocolumn, notitlepage,
...
%{Fv_mBBBAND0_GRB210704A_withgapbin.pdf}\put(-100,130){(b)}
    \caption{\label{fig:spectra} (Color online) (a) Left Y-axis with the unit of counts rate (s$^{-1}$): the histograms are the light curves of GBM detectors (NaI 5 and BGO 0, upper plot) and LAT (bottom plot); the high-energy photons are from the LAT data of TRANSIENT020 class (or evclass=16), and used in the analysis to maximize the statistics for spectral analysis of GRB prompt emission. Right Y-axis with the unit of MeV in bottom plot: energies of high-energy photons (denoted by circles).  The gray shadow in the first 2 s denotes intersection of all gaps between between sub-GeV (blue circles) and GeV (red circles) photons. 
...
To rigorously validate this spectral structure, we performed parameter estimation using Markov Chain Monte Carlo (MCMC) sampling with the \textsc{emcee} package~\cite{2013PASP..125..306F}. Similarly to \cite{2016MNRAS.463.1144W}, we compared a continuous power-law model with a model explicitly incorporating a spectral gap using the Bayesian Information Criterion (BIC). As illustrated in FIG.~\ref{fig:spectra} (b), the analysis yields a preference for the gap model with $\Delta \mathrm{BIC} \approx 5.7$. This value constitutes positive statistical evidence against a simple continuous spectrum~\footnote{$\Delta \mathrm{BIC}$ between 2 and 6 indicates positive evidence; 6--10 indicates strong evidence; and $>10$ implies very strong evidence.}, further supporting the physical separation of the two emission components.



\textit{Comoving Frame Energetics.}---To interpret the physical origin of the gap, we must determine the Lorentz factor $\Gamma$. The early prompt MeV emission ($<0.5$ s) is well-described by a non-dissipative photospheric model, allowing us to constrain the jet properties. A combined model of multicolor blackbody (mBB, e.g. see \cite[][]{2008ApJ...682..463P,2010ApJ...709L.172R,2018ApJ...866...13H}) plus a BAND function~\citep{1993ApJ...413..281B} (with a break energy at $\sim10^4$ keV) for the broad energy band is used to describe the spectrum of prompt emission from keV to GeV energy band; with a lowest BIC and difference $\Delta \rm{BIC}>10$, it is statistically preferred over the other models (see details in Figure~\ref{fig:spectra2} in Appendix.\ref{sec:spectra2}). Using the method described in \cite{2012MNRAS.420..468P, 2024ApJ...961..137S}, we derived $\Gamma$ from the quasi-thermal component of the spectrum. The estimated Lorentz factor evolves with time, peaking at $\Gamma \approx 300$ around the peak of luminosity and decaying to $\Gamma \approx 190$ one second later. 


Applying the Doppler correction $E' \approx E_{\rm obs} (1+z) / 2\Gamma$, where $z \simeq 0.27$ is the redshift derived from the Amati relation and jet modeling \cite{2023MNRAS.522.5204B}, the observed gaps translate to a comoving energy range of:
\begin{equation}
E_{\rm gap}^{\prime} \approx 2 - 7 \text{ MeV}.
\end{equation}
This energy range is critical. We first rule out standard electromagnetic mechanisms. A two-component jet model (a fast spine and slow sheath, see, e.g., Ref.~\cite{2025ApJ...995L...2A}) fails to explain the synchronized temporal evolution of the sub-GeV and GeV components. Similarly, Klein-Nishina suppression in Inverse Compton scattering would produce a high-energy cutoff rather than a discrete gap followed by a recovery in flux at $>1.2$ GeV.
...
$\varepsilon'_\gamma \simeq 4~{\rm MeV}$ in the deuteron rest frame \cite{IAEA2000}. Note that  $\Gamma\simeq 200$ is taken for the merged shells in internal shocks; it is typically less than the maximum value estimated by the quasi-thermal component.  
...
\end{equation}
where $\varepsilon'_\gamma$ is the photon energy in the comoving frame. One has $x \gtrsim 2.5\times 10^4$ for $\varepsilon_\gamma'=4$ MeV and $\gamma_m\gtrsim10^{3.5}$. In this Klein-Nishina limit, the total scattering cross section is suppressed to \cite{1986rpa..book.....R}
\begin{equation}
\sigma_{\rm KN} \simeq 
\frac{3}{8}\sigma_T
\frac{\ln(2x)+1/2}{x} 
...
\lesssim 0.6 E_{\rm K, 53}R_{12.5}^{-2} \Gamma_2^{-1}=0.5Y_{D,-1}^{-1} \tau_{\gamma D},
\label{eq:sigmaKN-GBM}
\end{equation}
with assuming local electrical neutrality in the jet, $n'_{e}\simeq n'_p$. The break energy ($\varepsilon_b$) in the spectrum of the non-thermal component described by a BAND function (e.g. the synchrotron emission by the relativistic electrons in magnetic field) may correspond to $\tau_{\rm KN} (\varepsilon_b')\sim1$ at $\gamma_e\sim\gamma_m$. From the results of the modeling of the non-thermal component, the break energy is $\varepsilon_b\sim10^4$ keV, which corresponds to $\gamma_m\sim10^{5.4}$ in this case; $\sigma_{\rm KN}\sim10^{-30}$ cm$^{2}$ is much lower for the photons of $\varepsilon_\gamma'=4$ MeV, due to a deeper Klein-Nishina regime.
...
\end{equation}
The freezeout $Y_{D}$ can be greater than a few percent or even as large as $10\%$ under the right conditions below the radius of GeV emissions (see the simulation in Ref.~\citep[][]{2002ApJ...580..368P}). The total energy of the outflow could be estimated by the radiative $E_{\gamma, \rm iso}$ and the radiative efficiency ($\epsilon_\gamma$) as
\begin{equation}
    E_{\rm tot, iso}= E_{\gamma, \rm iso}/\epsilon_\gamma;
\end{equation}
for baryonic fireball, $E_{\rm K}\lesssim (1-\epsilon_\gamma)E_{\rm tot, iso}$.
It is reasonable that $\epsilon_\gamma\lesssim10\%$, and this is because the radiation efficiency of internal shocks is typically low (e.g., see Refs.~\citep[][]{2011ApJ...726...90Z,2008FrPhC...3..306F}). $E_{\gamma, \rm iso}\simeq 10^{52}$ erg for GRB 210704A, therefore, we assume that $E_{\rm K}$ is comparable to $ 10^{53}$ erg. The internal shock radius is 
\begin{equation}
R \simeq 2 \Gamma^2 c \delta t \simeq 6 \times 10^{12} \Gamma_{2.5}^2 \delta t_{-3} \ {\rm cm},
\end{equation}
which has an order of magnitude of $10^{12}-10^{13}$ cm, because the time scale ($\delta t$) of pulse duration is determined to be about 1 ms by wavelet decomposition (with a sampling rate of 4000 Hz; see the details of the method in~\citep[][]{2013MNRAS.432..857M}); it lies in the typical range  for short GRBs~\cite{2014ARA&A..52...43B}. Thus, with given $Y_{D}\simeq0.1$, $E_{\rm K}\sim10^{53}$ erg and $R\sim10^{12.5}$ cm, equation (\ref{eq:criterion}) is satisfied. The baryon loading mass in the jet is calculated to be $f_{b}M_{\rm iso}\sim10^{-6}M_{\odot}$ by $E_{\rm K}=\Gamma M_{\rm iso}c^2$ (the typical beaming factor for short GRBs is $f_b\sim6\times10^{-3}$, see~\citep[][]{2014ARA&A..52...43B}), which is a reasonable value for merger events~\citep{2007PhR...442..166N,2019LRR....23....1M,2025PhRvD.111f3040S}.

\textit{Conclusion.}---We identified a statistically significant spectral gap in the prompt emission of GRB 210704A. Interpreting this as resonant absorption by deuterium nuclei in a jet with $\Gamma \sim 200-300$ for the first time, we find a consistent physical picture that links the burst to a neutron-rich progenitor. This represents the first direct spectroscopic detection of nucleosynthesis ingredients in the $r$-process in the prompt gamma-ray phase, opening a new window for studying nuclear astrophysics in extreme environments.
...
\caption{The Spearman rank correlation coefficients of temporal evolution between $\gamma^{\rm envelop}_{\rm GeV}$ and $\gamma^{\rm envelop}_{\rm sub-GeV}$ (denoted by different line styles and colors) extracted with different bin-edge division ratios. The gray shadow between 0.62 and 1.21 GeV shows the intersection of all gaps between sub-GeV and GeV photons.\label{fig:correlation}}
 \end{center}
\end{figure} 


\section{The possible correlation between $\gamma^{\rm envelop}_{\rm sub-GeV}$ and $\gamma^{\rm envelop}_{\rm GeV}$}\label{sec: correlation}

The Spearman rank correlation coefficient ($\rho$)~\citep{spearman}\footnote{The spearman rank correlation coefficient $\rho$ is defined as $\rho = \frac{\mathrm{cov}(r(x),r(y))}{\sigma_x \sigma_y} $ where $r(a)$ represents the rank of element $a$. For non-duplicated datasets, $\rho$ can be interpreted as $\rho = 1 - \frac{6 \Sigma d^2_i}{n(n^2-1)} $ where $d_i = r(x) - r(y)$.} is used to quantify the monotonic relationship of the temporal evolution between the energy of $\gamma^{\rm envelop}_{\rm GeV}$ (denotes the lower envelope of GeV photons) and that of $\gamma^{\rm envelop}_{\rm sub-GeV}$ (denotes the upper energy envelope of sub-GeV photons). $\gamma^{\rm envelop}_{\rm GeV}$ is naturally formed by the GeV photons as shown in FIG.~\ref{fig:correlation} in red circles connected by a red line.
In order to ensure a one-to-one correspondence between $\gamma^{\rm envelop}_{\rm sub-GeV}$ and $\gamma^{\rm envelop}_{\rm GeV}$, the maximum sub-GeV energy value in each time bin was extracted as $\gamma^{\rm envelop}_{\rm sub-GeV}$; the data binning in the range of $T_0+[0.70, 2.05]$ s is performed as follows: 1) the edges of the bin are determined to be the n-equal diversion points in time of each adjacent GeV point in $\gamma^{\rm envelop}_{\rm GeV}$; 2) to reduce the impact of artificial selection, the bin-edge division ratio (rate) is tested from 0.01 to 0.99 with an increment of 0.01 (for example, a rate equal to 0.2 means left five-division point). 
...
\section{Spectral Modelings with different models}\label{sec:spectra2}
...
FIG.~\ref{fig:spectra2} shows the time-integrated spectral modeling results with mBB + BAND, BAND, blackbody (BB) + BAND, and mBB + PL. 
 %By the Bayesian information criterion method~\citep[BIC,][]{2016MNRAS.463.1144W}, a combined model of multicolor blackbody~\citep[mBB, e.g. see details in][]{2008ApJ...682..463P,2010ApJ...709L.172R,2018ApJ...866...13H} plus a BAND function for the broad energy band is statistically preferred over the other models, as shown in Figure~\ref{fig:spectra2}. %It is evident that the component of a broad energy band has a different origin from that of the peaking component in MeV energy band. %It is similar to the case of 090902B~\citep{2009ApJ...706L.138A, 2010ApJ...709L.172R, 2012MNRAS.420..468P}, therefore we think the most peaking emission in the MeV energy band is quasi-thermal, and this conclusion is strengthened by the physical interpretation. 
...
\includegraphics[width=.47\textwidth]{Fv_mBBPL0_GRB210704A.pdf}\put(-110,130){(d) }
\caption{\label{fig:spectra2} (Color online) Modeling results with mBB+BAND, BAND, BB+BAND, and mBB+PL, with considering zero-count in the gap (marked in gray).}
    \end{center}
\end{figure*}
%\clearpage
\bibliography{LIV}
