...
%DIF ADD GRB210704A-kilonova_v3_songxy.tex                                                            Sun Jul 19 14:34:39 2026
...
With $z=2.34$, the Lorentz \DIFdelbegin \DIFdel{Factor }\DIFdelend \DIFaddbegin \DIFadd{factor }\DIFaddend is estimated to be $\gtrsim500$ around the peak flux \DIFdelbegin \DIFdel{with the photon of }\DIFdelend \DIFaddbegin \DIFadd{from the photon with the }\DIFaddend highest energy (16.09 GeV); the model-dependent method using the quasi-thermal component in the prompt phase proposes a range of 500 to 900 (see the details in Appendix~\ref{sec: estiGamma}).
...
%DIFDELCMD < %%%
\DIFdelend Applying the Doppler correction $E' \approx E_{\rm obs} (1+z) / 2\Gamma$ with $\Gamma=300$, the observed gap ([0.62, 1.21] GeV) translates to a comoving energy range of:
\begin{equation}
E_{\rm gap}^{\prime} \approx 3.5 - 6.7 \text{ MeV}.
\end{equation}
%DIF <  Even with $\Gamma=900$, the range is $1.2-2.2$ MeV. This energy range is critical.
\DIFaddbegin \DIFadd{Even with $\Gamma=900$, the range is $1.2-2.2$ MeV. This energy range is critical.
}
...
%DIF > %% TODO(team): the quantitative gamma-gamma constraints below (Gamma<~170 at 10^11 cm; Gamma~3000 for a feature centered at 1 GeV) follow Response (3) in the reply letter; please finalize them together with the ongoing rework of the Gamma estimates mentioned in Response (4).
...
If the \DIFaddbegin \DIFadd{gap indeed arises from }\DIFaddend resonant deuterium absorption\DIFdelbegin \DIFdel{almost fix }\DIFdelend \DIFaddbegin \DIFadd{, the resonance energy in turn almost fixes }\DIFaddend the Lorentz factor \DIFdelbegin \DIFdel{being }\DIFdelend \DIFaddbegin \DIFadd{to be }\DIFaddend around 300\DIFdelbegin \DIFdel{, we }\DIFdelend \DIFaddbegin \DIFadd{; we therefore }\DIFaddend choose $\Gamma = 10^{2.5}$ as a typical value in the following.
...
%DIF > %% TODO(team): Response (4) in the reply letter states that the previous Gamma estimates were problematic (one-zone model; compactness lower limit) and that this part is being reworked; please finalize the equations and numbers in this subsection and in the related statements of the main text.
...
%DIFDELCMD < %%%
\begin{displaymath}\DIFdel{%DIFDELCMD < \label{eq:Rgg}%%%
...
\DIFdelend \DIFaddbegin \DIFadd{where $\Delta t$ is the arrival time delay of high-energy photons relative to the MeV emission.
...
\subsection{\DIFdelbegin \DIFdel{Estimation based }\DIFdelend \DIFaddbegin \DIFadd{Two-zone constraints }\DIFaddend on \DIFdelbegin \DIFdel{statistical correlation}\DIFdelend \DIFaddbegin \DIFadd{the Lorentz factors}\DIFaddend }
\DIFdelbegin \DIFdel{There are empirical correltions between $\Gamma$ and $E_{\rm iso}$ and $L_{\rm iso}$:
}\DIFdelend \DIFaddbegin \label{sec:twozone}
%DIF > %% Numerical results in this subsection are produced by the script twozone_gamma_min.py in the repository.
...
\DIFadd{We follow the formalism of \mbox{%DIFAUXCMD
\cite{2011ApJ...726L...2Z} }\hskip0pt%DIFAUXCMD
with the corrections given in \mbox{%DIFAUXCMD
\cite{2023ApJ...956L..38G} }\hskip0pt%DIFAUXCMD
(the TeV zone there corresponds to our GeV zone here). The optical depth of a GeV photon with observed energy $E_{\rm G}$, emitted at $R_{\rm G}=2\Gamma_{\rm G}^2c\,\eta t_{\rm G}/(1+z)$ (where $t_{\rm G}$ is its observed arrival time and $\eta\leq1$ parameterizes the emission radius; $\eta=1$ corresponds to shells ejected over the full delay $t_{\rm G}$), is
}\begin{equation}\DIFadd{\label{eq:tautwozone}
