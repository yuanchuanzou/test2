...
### F. 双区计算定稿并写入正文/附录/回复信（最终轮，按邹老师定的方案）

物理图像定稿：**GeV/PL 成分来自更靠外的内激波**（非 Gao & Zou 2023 的外激波），t_G≈δt≈1 ms → R_G=2Γ_G²cδt/(1+z)~10¹² cm，与 τ_γD 的半径同量级；MeV 峰成分来自更内的内激波；氘与 GeV 光子同区。计算靶谱用 **Band 的 β_M=2.80**（16 GeV 的对产生阈值 ~1 MeV 在 E_p 之上；Eq 17 中的 α 即取 β_M）。

- **附录 C 双区小节整体重写**（旧的 R_G≳2×10¹⁵/η 版本作废）：给出解析式（Gao & Zou 2023 Eq 17 第一行，Γ_G,min≈1.0×10³，注明因阈值落在 Band 陡翼上属高估/上限）与**数值全式结果 Γ_G,min≈4.3×10²、R_G≈3.4×10¹² cm**（用 Gao & Zou 已发表代码算得，代码经其 Table 1 GRB 221009A 验证：297.7/161.7/139.4/113.0 vs 297.92/162.93/139.31/113.06）。按邹老师要求只给 16.09 GeV 的数值；1.21–4.39 GeV 仅定性说明约束更弱。
- **正文 Comoving Frame Energetics**：一区 "Γ≳500" 表述改为——准热法 500–900 归光球外流；GeV 区由双区分析给 Γ_G≳4×10² @ R_G≈3×10¹²。
- **正文多普勒换算**：补 Γ=430 时缺口映射到 2.4–4.7 MeV（仍含氘共振）；"共振定 Γ≈300" 改为 "(3–4)×10²"，Γ=10^2.5 典型值保留。
- **正文替代解释段**：撤掉旧的 Γ≲170@10¹¹/Γ≈3000 数字（属被否定的旧一区估计），改为"τ_γγ 随能量单调上升→只能产生截断而非带恢复的缺口 + 在双区允许的 Γ_G 下缺口能段 τ_γγ≪1"并指向附录。
- **附录一区小节**：去掉旧 TODO，结论句改为"一区假设下才得 Γ≳500"，并交接到双区小节。
- **response.docx**：Response (4) 中我此前插入的两段（R_G~2×10¹⁵ 旧版）替换为定稿版；Response (3) 末尾加 [Update] 段说明旧的 γγ 数字已被双区结果取代。
- **代码**（均已推送，可复现）：`twozone_crossing_gaozou.py`（主计算，含 221009A 验证）、`c_221009A.txt`（验证数据）、`crossing_twozone.py`/`self_compactness.py`/`twozone_gamma_min.py`（过程探索，含勘误注记）、`self_absorption_16GeV.py`（τ_self 分析）。
- **遗留（tex 中留 %%% TODO）**：(1) 正文典型值 Γ=10^2.5 略低于 16 GeV 要求的 4.3×10²，且在 Γ≈434/R≈3.4×10¹² 处 τ_γD 需靠单脉冲 L_K 增强（或更大 Y_D）维持 ≳1——数值协调待定；(2) τ_self（16 GeV 对外区自身 PL 光子）由"外区自身 ~MeV 光度很弱"解决，需谱拟合佐证，邹老师另行补充。

### E. 双区（two-zone）洛伦兹因子致密性计算（第一版，已被 F 节取代，记录保留）
